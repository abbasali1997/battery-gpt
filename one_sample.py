...
cycles = 10
chunk_tokens = 3600 # 10 cycles per chunk
out_file = rf"samples/long_generation_10cycles.pkl"
...
if os.path.exists(out_file):
	...
	print(f"Resuming from {len(generated_ids)} total tokens")
else:
	generated_ids = encode(start) if not isinstance(start, list) else start
	print(f"Resuming from {len(generated_ids)} prompt tokens")    

total_out_tokens = cycles * 3600 + max_new_tokens

while len(generated_ids) < total_out_tokens:
	remaining = total_out_tokens - len(generated_ids)
	this_chunk = min(chunk_tokens, remaining)

	x = torch.tensor(generated_ids, dtype=torch.long, device=device)[None, ...]
	...
	print(f"Saved checkpoint: {len(generated_ids)} total tokens")	
	
	
decodeMessage = decode(generated_ids)
with open(f"samples/samples_10.bin", "wb") as f:
	pickle.dump(decodeMessage, f)
